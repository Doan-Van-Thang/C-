...
int choose_next_hop(int from, int to) {
  ...
  return table.empty() ? -1 : table[randint(0, int(table.size()) - 1)];
...
int main() {
  assert(std::freopen("result_links_falied", "a", stdout));
  std::cout << "\n\n\n\n\n\n\n";
  ...
  const int links_falied_prob = 2 /* / 100 */;

  for (int u = 0; u < n + m; u++) {
    for (int v : g[u]) {
      if (v > u && randint(1, 100) > links_falied_prob) links.emplace_back(u, v);
    }
  }
